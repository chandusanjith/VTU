...
from .models import MasterSemesters,MasterBranches,MasterNotes, MasterSubjects, MasterQuestionPapers, MasterVideoLab, DeviceAuth, AppVersion, AppForceUpdateRequired, MasterSyllabusCopy
from .serializers import NotesSerializer, NotesMasterSerializer, SubjectSerializer, QuestionPaperSerializer, MasterVideoLabSerializer, LoadSyllabusCopySerializer
...
class SnippetList(APIView):
  ...
  def get(self, request, sem, branch, subject, device_auth,format=None):
    if AuthRequired(device_auth) == True:
      ...
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(notes_serializer.data)
...
class QuestionPaperList(APIView):
  ...
  def get(self, request, sem, branch, subject, device_auth, format=None):
    if AuthRequired(device_auth) == True:
      ...
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(question_paper_serializer.data)
      return Response(return_json, status=status.HTTP_200_OK)
    else:
      return Response({"ERROR":"Access Denied"}, status=status.HTTP_404_NOT_FOUND)
...
class FetchMasterList(APIView):
  ...
  def get(self, request,device_auth, format=None):
    if AuthRequired(device_auth) == True:
      ...
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(serializer.data)
...
class FetchSubject(APIView):
  ...
  def get(self, request, sem, branch, device_auth, format=None):
    if AuthRequired(device_auth) == True:
      ...
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(sub_serializer.data)
      return Response(return_json, status=status.HTTP_200_OK)
    else:
      return Response({"ERROR":"Access Denied"}, status=status.HTTP_404_NOT_FOUND)
...
class LabManualVid(APIView):
  def get(self, request, sem, branch, subject, program_id, device_auth, format=None):
    if AuthRequired(device_auth) == True:
      ...
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(video_serializer.data)
      return Response(return_json, status=status.HTTP_200_OK)
    else:
      return Response({"ERROR":"Access Denied"}, status=status.HTTP_404_NOT_FOUND)
...
class LoadSyllabusCopy(APIView):
  def get(self, request, device_auth, format=None):
    if AuthRequired(device_auth) == True:
      mapped_key = DeviceAuth.objects.filter(device_key = device_auth)
      syllabus_master = MasterSyllabusCopy.objects.all()
      if not syllabus_master:
        return Response({'Auth_key':mapped_key[0].mapped_key,
        "ERROR":"404 NO DATA FOUND :("}, status=status.HTTP_404_NOT_FOUND) 
      syllabus_serilizer = LoadSyllabusCopySerializer(syllabus_master, many=True)
      return_json = {'Auth_key':mapped_key[0].mapped_key}
      return_json.update(syllabus_serilizer.data)
      return Response(return_json, status=status.HTTP_200_OK)
    else:
      return Response({"ERROR":"Access Denied"}, status=status.HTTP_404_NOT_FOUND)
